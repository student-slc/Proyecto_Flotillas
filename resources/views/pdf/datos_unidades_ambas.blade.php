@php
    ...
    $unidad = Unidade::where('cliente', '=', $cliente)->get();
    if ($cliente == 'todos') {
        ...
        foreach ($unidad as $unidade) {
            if ($unidade->tipo == 'Unidad Vehicular') {
                $cadena_1 = $cadena_1 . '<option value="' . strval($unidade->serieunidad) . '">' . strval($unidade->serieunidad) . '</option>';
            }
            if ($unidade->tipo == 'Unidad Habitacional o Comercial') {
                $cadena_1 = $cadena_1 . '<option value="' . strval($unidade->direccion) . '">' . strval($unidade->direccion) . '</option>';
            }
        }
    }
